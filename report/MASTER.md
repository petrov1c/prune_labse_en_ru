...
| *models/tune_model.pth*       | 0.7904 | 0.6511 | ***0.4377*** | ***0.7633*** | 0.9403 | 0.7583 | 0.7838 | 0.765  |
...
Результаты по среднеквадратичному значению критериев и вычислительной нагрузке (жирным обозначены средние значения, курсивом обозначена финальная модель):

| model                       |  Mean S |   CPU |size      | 
|:----------------------------|:--------:|:-----:|:----------|
| cointegrated/LaBSE-en-ru    | 0.739 |  20  | **490.0** |
| sentence-transformers/LaBSE |  **0.739** |  **19.7** |   1796.0    |
| *models/tune_model.pth*       |   **0.736** |  **12.3** |   **327.0** |
| cointegrated/rubert-tiny2   |   **0.704** |   **1.7**   | **111.0** | 
| models/prune_model.pth      |   0.647  |   12.5 |  327.0     |
...
Текущий результат был получен в ограничениях как по вычислительным ресурсам (NVIDIA GeForce RTX 3090), так и по времени (несколько суток проведения экспериментов). Однако, он вполне сопоставим с результатами известных работ, описанных [пункте 3](#Related). Более того, по некоторым метрикам был получен [SotA](#sota) на задаче, по которой есть previous art (то есть опубликованные статьи на этом датасете по этой задаче).
